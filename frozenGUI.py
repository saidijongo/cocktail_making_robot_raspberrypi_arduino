...
def start_pump(motor_pin, volume):
    ...
    try:
        ...
        while pump_running and (time.time() - start_time) < run_time:
            pass
...
pump_running = False  # Flag to track pump operation status
...
ingredient_motor_dropdown.grid(row=0, column=0, padx=7, pady=7)
...
LED_waiting_button = ttk.Button(order_frame, text="LED Waiting", command=lambda: send_command_to_arduino("WAITING"))
LED_waiting_button.grid(row=8, column=0, padx=10, pady=10)

LED_off_button = ttk.Button(order_frame, text="LED Off", command=lambda: send_command_to_arduino("FINISHED"))
...
volume_dropdown.grid(row=0, column=1, padx=7, pady=7)
